...
The target structural IVQR coefficient alpha_tau satisfies

    P(Y <= D alpha_tau + X_S0' beta_tau | X, Z) = tau

where S0 is the true active control set from the data-generating process. This
oracle estimator is infeasible in applications because S0 is known only in the
simulation. It is kept as a benchmark for feasible estimators.

Inference uses CH inverse-IVQR over the alpha grid A:
...
from ivqr.ch_inverse import estimate_ch_ivqr_controls
