"""Thesis-ready CSV table builders for Monte Carlo summaries."""
...
def _round_numeric(df: pd.DataFrame, columns: list[str], round_digits: int | None) -> pd.DataFrame:
...
def add_estimator_labels(df: pd.DataFrame) -> pd.DataFrame:
    ...
    labeled["estimator_label"] = labeled[ESTIMATOR_COLUMN].map(ESTIMATOR_LABELS).fillna(
        labeled[ESTIMATOR_COLUMN]
    )

    estimators = labeled[ESTIMATOR_COLUMN].astype(str)
    unknown = sorted(estimator for estimator in estimators.unique() if estimator not in ESTIMATOR_ORDER)
    ...
    return labeled.sort_values(TABLE_GROUP_COLUMNS + [ESTIMATOR_COLUMN]).reset_index(drop=True)
...
def make_diagnostic_table(
    ...
    _validate_summary_columns(summary)
    table = add_estimator_labels(summary)
    available_diagnostics = [column for column in DIAGNOSTIC_COLUMNS if column in table.columns]
    columns = TABLE_GROUP_COLUMNS + [ESTIMATOR_COLUMN, "estimator_label"] + available_diagnostics
    table = table[columns].copy()
...
def write_tables(
    ...
    comparison_metrics = [metric for metric in CORE_METRICS if metric in summary.columns]
    ...
    make_diagnostic_table(summary, round_digits=round_digits).to_csv(diagnostic_path, index=False)
